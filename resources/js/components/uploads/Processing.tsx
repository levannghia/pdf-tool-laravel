...
function Processing({ title, token, setRecentlySuccessful }: {
    title: string,
    token: string,
    ...
    useEffect(() => {
        window.Echo.channel(`task-processing-${token}`)
            .listen(".task-processing", (data: { percentage: number }) => {
                setHasPercentage(true);
                setPercentage(data.percentage)
            })

        window.Echo.channel(`task-processed-successfully-${token}`)
            .listen(".task-processed-successfully", () => router.get(route("download_file", token)));

        window.Echo.channel(`task-processing-failed-${token}`)
            .listen(".task-processing-failed", (data: { message: string }) => {
